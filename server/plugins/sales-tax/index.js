...
internals.getSalesTaxAmount = (params) => {
    return new Promise((resolve, reject) => {
        ...
        if(params.countryCodeAlpha2 === 'US') {
            switch(params.state) {
                case 'CA':
                    // NOTE: shipping is not taxable in CA
                    taxAmount = parseFloat(params.subtotal) * parseFloat(process.env.TAX_RATE_CALIFORNIA || '0.09');
                    break;
            ...
            config: {
                ...
                validate: {
                    payload: {
                        ...
                        subtotal: Joi.number().precision(2).positive().max(99999999.99).required(),
